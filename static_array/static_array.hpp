...
namespace csb
{
    ...
    template<ccept::Regular T, std::size_t n>
    class static_array
    {
        public:
        using value_type = T;
        ...
        constexpr static_array(static_array  const&) 
            noexcept(std::is_nothrow_copy_constructible_v<value_type>) = default;
        constexpr static_array(static_array  &&) 
            noexcept(std::is_nothrow_move_constructible_v<value_type>) = default;
        constexpr static_array& operator=(static_array const&) 
            noexcept(std::is_nothrow_copy_assignable_v<value_type>) = default;
        constexpr static_array& operator=(static_array &&)
            noexcept(std::is_nothrow_move_assignable_v<value_type>) = default;
        
        ~static_array() 
            noexcept(std::is_nothrow_destructible_v<value_type>) = default;

        // should I offer this noexpetness in case I end up using lazy initialization??
        constexpr static_array(value_type const&all)
            noexcept(
        ...
        constexpr static_array(std::initializer_list<T> const&il)
            noexcept(
                std::is_nothrow_default_constructible_v<value_type>
                && std::is_nothrow_copy_assignable_v<value_type>)
                : _store{}
        ...
        { static_assert(i  > 0 && i < n, "out of bounds"); return _store[i]; }

        constexpr reference operator[](size_type i) noexcept
        { return _store[i];}
        template<auto i>
        constexpr reference operator[](idx_t<i>) noexcept
        { static_assert(i  > 0 && i < n, "out of bounds"); return _store[i]; }
        ...
        {
            if (i >= n) throw std::out_of_range("static_array::at() range error");
            return _store[i];
        }
        ...
        constexpr void fill(value_type const& with) 
        ...
        constexpr void init(R const& rng)
        {
            ...
            using std::end;
            auto me = this->begin();
            for (auto rit = begin(rng); 
                me != this->end() && rit != end(rng);
                ++me, ++rit)
            {
                *me = *rit;
            }
