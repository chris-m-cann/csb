...
namespace csb
{
    ...
    SCENARIO("Redblack tree Deletion")
    {
              ...
              "child,r, is red")
        {
            ...
            auto otherRed = make_node(35, red(), s.get());

            s->right = std::move(r);
            s->left = std::move(otherRed);
    ...
    SCENARIO("Red black Fuzz deletions")
    {
        ...
        GIVEN("a red black tree with a random list of integers")
        {
            ...
            WHEN("deleting random nodes")
            {
                ...
                while (values.size() > 1)
                {
                    std::uniform_int_distribution<> dis2(0, values.size());
